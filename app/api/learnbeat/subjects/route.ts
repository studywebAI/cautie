    ...
    const processedSubjects = subjects?.map(subject => {
      ...
      subject.chapters?.forEach(chapter => {
        chapter.paragraphs?.forEach(paragraph => {
          ...
          const userProgress = paragraph.progress_snapshots?.find(p => p.student_id === user.id);
      ...
      subject.chapters?.forEach(chapter => {
        chapter.paragraphs?.forEach(paragraph => {
          allParagraphs.push({
            ...
            progress: paragraph.progress_snapshots?.find(p => p.student_id === user.id)?.completion_percent || 0
          });
        });
      });
